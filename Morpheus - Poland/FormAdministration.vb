        ...
        Try
            ...
            If listFile <> "" Then
                mailSender("STATUS" & "_SignTo", "STATUS" & "_SignCopy", "Automatic SrvDoc Message:" & vbCrLf & vbCrLf & _
                           vbCrLf & "Please CHECK the new file / version in the server : " & " " & vbCrLf & vbCrLf & listFile & vbCrLf & vbCrLf & "Best Regard", "File changes notification  " & _
    ...
    Sub EcrMailScheduler()
        ...
        For Each row In RowSearchEcr
            ...
            If readDocSign(row("iddoc").ToString, refresh) = "" Then
                ...
                If InStr(row("Asign").ToString & row("Rsign").ToString & row("Lsign").ToString & row("Usign").ToString & row("Bsign").ToString & row("Esign").ToString & row("Nsign").ToString & row("Psign").ToString & row("Qsign").ToString, "CHECKED", CompareMethod.Text) <= 0 And _
                    DateDiff(DateInterval.Day, Now, dt) < 2 Then
